...
from stereacl.interventions import make_rank_k_projection_hook
    ...
    try:
        ...
        for k in ks:
            for axis in axes:
                ...
                # Apply at all layers where we have direction data for this axis.
        ...
        write_csv(out_path, rows, fieldnames=["axis", "k", "rank_used", "stereotype_score", "mean_margin", "n_pairs"])
        ctx.register_artifact(out_path, artifact_type="table", description="Rank-sweep causal curves.")

        complete_run(ctx, metrics={"rows": len(rows), "ks_swept": ks, "axes": len(axes), "dry_run": False})
