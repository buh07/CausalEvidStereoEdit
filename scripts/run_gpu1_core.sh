#!/bin/bash
# GPU 1: Gemma-2-2B-IT core validity pack prerequisites + optional B1 sanity + D1
set -euo pipefail
...
MIX_E1="results/01_layerwise_probing/2026-05-07/run-017"
MIX_E2="results/02_component_dla/2026-05-07/run-017"
MIX_E3="results/03_attribution_patching/2026-05-07/run-022"

rm -f results/core_done_g1.flag

echo "=== GPU1 CORE START $(date) ==="
...
echo "--- Exp11 optional sanity (Gemma-IT gender/profession sweep) ---"
python experiments/11_hydra_multisite.py \
  --model "$MODEL" --device "$DEVICE" --torch-dtype "$DTYPE" \
  --heldout-pairs 120 --n-sites "8,12,16,20" \
  --promoters-only --ranking-source union \
  --axes "gender,profession" \
  --bootstrap-n 1000 \
  --eval-sources "stereoset_intrasentence,crows_pairs" \
  --exp1-run-dir "$MIX_E1" --exp2-run-dir "$MIX_E2" --exp3-run-dir "$MIX_E3" \
  --seed 52

echo "--- Exp15 D1 matrix (Gemma-IT) ---"
...
touch results/core_done_g1.flag
echo "=== GPU1 CORE DONE $(date) ==="
