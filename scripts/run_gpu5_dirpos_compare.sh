...
LOG=results/log_gpu5_dirpos_compare.txt
mkdir -p results
export HF_TOKEN="${HF_TOKEN:?Set HF_TOKEN}"
echo "[$(date -Iseconds)] START gpu5 gemma-it" | tee "$LOG"
CUDA_VISIBLE_DEVICES=5 python experiments/01_layerwise_probing.py \
...
CUDA_VISIBLE_DEVICES=5 python experiments/04_ablation_validation.py \
  --model google/gemma-2-2b-it \
  --device cuda \
  --torch-dtype bfloat16 \
  --heldout-pairs 120 \
...
echo "[$(date -Iseconds)] DONE gpu5 gemma-it" | tee -a "$LOG"
