...
if [[ $# -ne 3 ]]; then
  echo "Usage: $0 <model_key> <model_name> <gpu_id>" >&2
  exit 2
fi
...
log_msg "Lane start | model_key=$MODEL_KEY model=$MODEL_NAME gpu=$GPU_ID run_tag=$RUN_TAG"

DEVICE="$(cfg_get global.device)"
DTYPE="$(cfg_get global.torch_dtype)"
MAXLEN="$(cfg_get global.max_length)"
HELDOUT="$(cfg_get global.heldout_pairs)"
...
# Distinct seeds for source-specific ranking runs to keep run lookup unambiguous.
SEED_SS=29
SEED_CR=47
...
# Exp09 strict adjudication with deterministic shuffle-before-truncate default
...
# Source-specific rankings for Exp15 transfer matrix
...
# Point 1 same-position-valid AR replication
run_cmd "exp26-ar-same-position" "$PYTHON_BIN experiments/26_ar_same_position_replication.py --model '$MODEL_NAME' --device '$DEVICE' --torch-dtype '$DTYPE' --heldout-pairs $HELDOUT --train-pairs $E26_TRAIN --bootstrap-n $BOOT --exp1-run-dir '$E1_MIX_DIR' --max-length $MAXLEN --seed $SEED_CANON --emit-pair-level"
...
# Point 6 multi-token robustness extension
...
# Exp28 multi-token matched inject/remove asymmetry (Exp27b)
run_cmd "exp28-multitoken-matched" "$PYTHON_BIN experiments/28_multitoken_matched_asymmetry.py --model '$MODEL_NAME' --device '$DEVICE' --torch-dtype '$DTYPE' --heldout-pairs $HELDOUT --eval-scope '$E28_SCOPE' --max-span-len $E28_MAX_SPAN --span-strata '$E28_SPAN_STRATA' --bootstrap-n $BOOT --power-alpha $E28_ALPHA --target-power $E28_POWER --sesoi $E28_SESOI --exp1-run-dir '$E1_MIX_DIR' --max-length $MAXLEN --seed $SEED_CANON"
E28_DIR="$(latest_run_by_seed '28_multitoken_matched_asymmetry' "$MODEL_NAME" "$SEED_CANON")"
write_state "${MODEL_KEY}_exp28_run_dir" "$E28_DIR"

# Point 4 seed-aggregated core analysis inputs (Exp16 with fixed seeds)
for seed in "${SEEDS_AGG[@]}"; do
  run_cmd "exp16-seed-${seed}" "$PYTHON_BIN experiments/16_asymmetry_matrix.py --model '$MODEL_NAME' --device '$DEVICE' --torch-dtype '$DTYPE' --heldout-pairs $HELDOUT --bootstrap-n $BOOT --position-only --emit-pair-level --exp1-run-dir '$E1_MIX_DIR' --max-length $MAXLEN --seed $seed"
...
# Composition-sensitivity Exp16 reruns: identical heldout (mixed eval) with source-specific direction pools
...
# Transfer positive control: source-specific direction injection 2x2 matrix
...
# Point 10 lightweight prompt comparator calibration
run_cmd "exp29-prompt-calibration" "$PYTHON_BIN experiments/29_prompt_calibration_baseline.py --model '$MODEL_NAME' --device '$DEVICE' --torch-dtype '$DTYPE' --heldout-pairs $HELDOUT --bootstrap-n $BOOT --exp1-run-dir '$E1_MIX_DIR' --exp16-run-dir '$E16_CANON' --mitigation-prefix '$E29_PREFIX' --max-length $MAXLEN --seed $SEED_CANON"
...
write_state "${MODEL_KEY}_label" "$(cfg_get core_models.$GPU_ID.label 2>/dev/null || echo "$MODEL_KEY")"
