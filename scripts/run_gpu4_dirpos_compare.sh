...
LOG=results/log_gpu4_dirpos_compare.txt
mkdir -p results
export HF_TOKEN="${HF_TOKEN:?Set HF_TOKEN}"
echo "[$(date -Iseconds)] START gpu4 gemma" | tee "$LOG"
CUDA_VISIBLE_DEVICES=4 python experiments/01_layerwise_probing.py \
...
CUDA_VISIBLE_DEVICES=4 python experiments/04_ablation_validation.py \
  --model google/gemma-2-2b \
  --device cuda \
  --torch-dtype bfloat16 \
  --heldout-pairs 120 \
...
echo "[$(date -Iseconds)] DONE gpu4 gemma" | tee -a "$LOG"
