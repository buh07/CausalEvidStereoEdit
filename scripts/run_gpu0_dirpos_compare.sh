...
LOG=results/log_gpu0_dirpos_compare.txt
mkdir -p results

echo "[$(date -Iseconds)] START gpu0 gemma" | tee "$LOG"
CUDA_VISIBLE_DEVICES=0 python experiments/01_layerwise_probing.py \
...
for p in glob.glob('results/01_layerwise_probing/*/run-*/manifest.json'):
    ...
    if params.get('model')!='google/gemma-2-2b':
        continue
...
CUDA_VISIBLE_DEVICES=0 python experiments/04_ablation_validation.py \
  --model google/gemma-2-2b \
  --device cuda \
  --torch-dtype bfloat16 \
  --heldout-pairs 120 \
...
echo "[$(date -Iseconds)] DONE gpu0 gemma" | tee -a "$LOG"
