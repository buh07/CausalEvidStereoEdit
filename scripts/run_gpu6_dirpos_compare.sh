...
LOG=results/log_gpu6_dirpos_compare.txt
mkdir -p results
export HF_TOKEN="${HF_TOKEN:?Set HF_TOKEN}"
echo "[$(date -Iseconds)] START gpu6 llama" | tee "$LOG"
CUDA_VISIBLE_DEVICES=6 python experiments/01_layerwise_probing.py \
...
CUDA_VISIBLE_DEVICES=6 python experiments/04_ablation_validation.py \
  --model meta-llama/Llama-3.2-3B \
  --device cuda \
  --torch-dtype bfloat16 \
  --heldout-pairs 120 \
...
echo "[$(date -Iseconds)] DONE gpu6 llama" | tee -a "$LOG"
